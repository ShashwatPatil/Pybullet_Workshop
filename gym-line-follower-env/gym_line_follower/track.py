...
def bernstein(n, k, t):
    return binom(n, k) * t ** k * (1. - t) ** (n - k)  ## nCk * (t)^k * (1-k)^(n - k)


def bezier(points, num=200):
    N = len(points)
    t = np.linspace(0, 1, num=num)   #creats a normal distribution over the inteval of 0 to 1 with having 200 points in normal curve 
